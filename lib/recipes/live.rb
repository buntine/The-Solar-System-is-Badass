...
namespace :live do
  ...
  task :view_log do
    n = (variables[:n] || "100").to_s

    if n =~ /^\d+$/
      run "tail -n #{n} #{current_path}/log/production.log"
    else
    ...
    def local_dump(server)
      if using_mysql?
        if target_server == server
          ...
          user = '-u '+user unless user.nil?
          pass = '-p'+pass unless pass.nil?
          `cat cap_dump_data.sql | mysql #{user} #{pass}`
  ...
  namespace :git do
    desc "Prints the last n commit messages."
    task :log do
      n = (variables[:n] || "5").to_s
      run "cd #{current_release}; git log -#{n}"
    end
  end
