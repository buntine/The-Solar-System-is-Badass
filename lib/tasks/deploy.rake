...
# Attempts to fetch the repository name from our VCS server.
def fetch_repo_name
...
namespace :deploy do
  ...
  task :setup do
    ...
    puts "\t- Constructed deployment config (config/deployment_config.yml)"
    ...
    puts "\t- Generated database config (config/database.yml)"
